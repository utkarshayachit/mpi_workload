...
namespace
{
    void thread_func(const int rank, const int thread_id, const int num_ranks, const int num_threads, const size_t buffer_size, const int interval, MPI_Comm comm)
    {
        ...
        while (true)
        {
            ...
            const auto status = MPI_Alltoall(MPI_IN_PLACE, 0, 0,
                recv_buffer.data(), num_ints, MPI_INT, comm);
            ...
            {
                const auto progress_elapsed_time = std::chrono::duration_cast<std::chrono::seconds>(end_time - last_progress_time);
